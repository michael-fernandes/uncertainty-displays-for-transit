...
Other misc differences: dot20

```{r}
dot20_mean_diffs = last_trial %>%
  mutate(vis = fct_relevel(vis, "dot20")) %>%
  compare_levels(mu, by = vis, comparison = control) 
...
dot20_mean_diffs %>%
  ungroup() %>%
  mutate(vis = factor(vis, levels = c(levels(vis), "dot20 - dot20"))) %>%
  bind_rows(data_frame(vis = factor("dot20 - dot20", levels = levels(.$vis)), mu = 0)) %>%
  ggplot(aes(y = vis, x = mu)) +
  geom_vline(xintercept = 0, color = "black", linetype = "dashed") +
  geom_halfeyeh(fun.data = median_qih, fatten.point = 1.3) +
  coord_cartesian(xlim = c(-0.11, 0.01), ylim = c(1, 10.5))
...
dot20_prec_diffs %>%
  ungroup() %>%
  mutate(vis = factor(vis, levels = c(levels(vis), "dot20 - dot20"))) %>%
  bind_rows(data_frame(vis = factor("dot20 - dot20", levels = levels(.$vis)), sd = 0)) %>%
  ggplot(aes(y = vis, x = sd)) +
  geom_vline(xintercept = 0, color = "black", linetype = "dashed") +
  geom_halfeyeh(fun.data = median_qih, fatten.point = 1.3) +
